...
ChatAnalysis is a CLI pipeline that turns Claude Code chat transcripts into structured analysis: evidence extraction, episode reconstruction, semantic clustering, and LLM-backed skill mining.
...
1. `chatanalysis discover` — find projects and sessions
2. `chatanalysis parse` — extract raw events from transcripts
3. `chatanalysis normalize` — classify into canonical evidence
4. `chatanalysis build-view` — create filtered corpus views
5. `chatanalysis search` — query evidence
6. `chatanalysis detect` — run pattern detectors
7. `chatanalysis report` — generate analysis reports
8. `chatanalysis extract-episodes` — reconstruct multi-turn episodes
9. `chatanalysis semantic-run` — optional: embeddings, clustering, skill-mining
...
```
CHATANALYSIS_LLM_BASE_URL    # OpenAI-compatible endpoint (default: https://api.openai.com/v1)
CHATANALYSIS_LLM_API_KEY     # API key for skill mining
CHATANALYSIS_LLM_MODEL       # Model name (default: gpt-4o-mini)
CHATANALYSIS_EMBEDDING_MODEL  # Sentence-transformer model (default: all-MiniLM-L6-v2)
```
...
1. Add the view definition in `src/chatanalysis/views/definitions.py`
2. Register in `VIEW_DEFINITIONS` dict
3. The view is automatically available via `chatanalysis build-view --view your_view_name`
...
- **"CHATANALYSIS_LLM_API_KEY not set"** — Copy `.env.example` to `.env` and add your API key. Only needed for `semantic-run --method skill-mining --backend pydantic-ai`.
